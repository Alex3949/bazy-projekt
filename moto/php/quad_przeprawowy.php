...
<table style="width: 95%">
    <tr>
        <th colspan="9"><h2>Quady przeprawowe:</h2></th>
    ...
    $db->set_charset("utf8");
    $find_data= "select * from rates_quad where typ = 'Przeprawowy'";
    $sql = "select * from quad where typ = 'Przeprawowy'";
    $result= $db->query($sql);
    ...
    if($result-> num_rows>0 and $result2-> num_rows>0){
        while($row=$result->fetch_assoc() AND  $row3=$result2->fetch_assoc() ){
            ...
            echo     "<tr><td>" . $row["marka"] . "</td><td>" . $row["model"] . "</td><td>" . $row["rok_produkcji"] . "</td><td>" . $row["moc"] . "</td><td>" . $row["poj_silnika"] . "</td><td>" . $row["mom_obrot"] . "</td><td>" . $row["typ"] .  "</td><td>" ."<form action='ocena_quad_przeprawowy.php' method='POST'>
