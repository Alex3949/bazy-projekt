...
<head>
    ...
    <title>Motocykle turystyczne</title>
    <link rel="stylesheet" href ="../css/maszyny2.css">
...
<table style="width: 95%" >
    <tr>
        <th colspan="9"><h2>Motocykle turystyczne:</h2></th>
    ...
    $db->set_charset("utf8");
    $find_data= "select * from rates where typ='Turystyczny' ";
    $sql = "select * from motocykl where typ = 'Turystyczny'";
    $result= $db->query($sql);
    ...
    if($result-> num_rows>0 and $result2-> num_rows>0){
        while($row=$result->fetch_assoc() AND  $row3=$result2->fetch_assoc() ){
            ...
            $hits=$row3['hits'];
                ...
                echo     "<tr><td>" . $row["marka"] . "</td><td>" . $row["model"] . "</td><td>" . $row["rok_produkcji"] . "</td><td>" . $row["moc"] . "</td><td>" . $row["poj_silnika"] . "</td><td>" . $row["mom_obrot"] . "</td><td>" . $row["typ"] .  "</td><td>" ."<form action='ocena.php' method='POST'>
